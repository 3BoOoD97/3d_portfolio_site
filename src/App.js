...
function App() {
    ...
    div > <
        ...
        My name is Abdalrhman Dabor I am 25 years old.Currently, I live in Sweden.I received my BSc in Software Technology from Linnéuniversitetet in Sweden and I have 1 year as an IT Technician In my current role at ATEA I am responsible
    for providing a wide range of services to support, fix, and maintain the hardware and software of an organisation’ s computer systems.Including installing, troubleshooting, testing, helping roll out security features, hardware, and software, or maintaining networks and computer systems.

    My primary goal is to apply my technical expertise all throughout the full software life cycle to ensure the production and delivery of products and services that meet client specifications.Along with a competent software development team, and strong personal knowledge, skills, and experience in software engineering, I am positive that this goal can be achieved.My experience as a junior software developer enhanced my abilities in designing, implementing, testing, and upgrading software.One of my objectives is to keep updated with the latest IT trends and technologies.I am confident that
    if given the opportunity, I can be a valuable talent to the company.

    Please find my resume attached.Feel free to contact me
    if you have any further questions you would like to discuss.
    ...
    section className = "w-full flex items-center justify-center" >
        ...
        VerticalTimeline > {
            ...
            Experience.map((n) => ( <
                ...
                date = { n.date }
                iconStyle = {
                    { background: "rgb(21, 24, 31)", color: "#888" }
                }
                icon = { n.iconsSrc } >
                <
                h3 className = "vertical-timeline-element-title" > { n.title } <
                /h3> <
                h4 className = "vertical-timeline-element-subtitle" > { n.location } <
                /h4> <
                p > { n.description } < /p> < /
                VerticalTimelineElement >
            ))
    ...
    id = "projects" > {
            ...
            Projects.map((n, i) => ( <
                ...
                <
                p className = "text-lg text-textBase font-medium uppercase" > { n.name.length > 25 ? `${n.name.slice(0, 25)}...` : n.name } <
                /p>
                ...
                Technologies <
                span className = "block text-sm text-gray-500" > { n.techs } <
                /span> < /
                p > <
                a href = { n.github } >
                <
    ...
    div className = "flex items-center justify-evenly w-full my-4 flex-wrap gap-4" > {
            ...
            SocialLinks.map((n) => ( <
                ...
                href = { n.link }
                key = { n.id }
                className = "w-full md:w-auto px-3 md:px-8 py-5 border border-zinc-800 rounded-2xl hover:border-zinc-600 duration-100 ease-in-out cursor-pointer flex items-center justify-center gap-3" > { n.iconSrc } <
